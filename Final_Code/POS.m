function [H_f, H_b, MAE, Rounded_MAE, pred_pulse_rate_f, pred_pulse_rate_b] = POS(video1, video2, data, bbox_f, bbox_b)
...
l_f = 48; % l=32 (20 frames/s camera)
...
for i = 1:length_video_f
    ...
    window_pred_pulse_rate_f = pred_heart_rate(window_pred_bvp_f,frame_rate_f);
    window_pred_pulse_rate_f(1,i) = window_pred_pulse_rate_f;
    pred_bvp_f = [pred_bvp_f(frame_rate_f:end)];
end
pred_pulse_rate_f = pred_heart_rate(H_f,frame_rate_f);
...
l_b = 48; % l=32 (20 frames/s camera)
...
for i = 1:length_video_b
    ...
    window_pred_pulse_rate_b = pred_heart_rate(window_pred_bvp_b,frame_rate_b);
    window_pred_pulse_rate_b(1,i) = window_pred_pulse_rate_b;
    pred_bvp_b = [pred_bvp_b(frame_rate_b:end)];
end
pred_pulse_rate_b = pred_heart_rate(H_b,frame_rate_b);
...
MAE = sqrt(mean_error.^2);
Rounded_MAE = mean(error);
end
