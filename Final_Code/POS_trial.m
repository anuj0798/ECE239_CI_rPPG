function [H_f, H_b, MAE,Rounded_MAE, pred_pulse_rate_f, pred_pulse_rate_b] = POS_trial(video1, video2, data, bbox_f, bbox_b)
...
while hasFrame(input_video_f) && (input_video_f.CurrentTime <= length_video_f)
    ...
    if(skin_segmentation) %skin segmentation - originally specified in reference as an OC-SVM from Wang et al. 2015
...
pred_pulse_rate_f = prpsd(H_f,frame_rate_f,60,100);
...
pred_pulse_rate_b = prpsd(H_b,frame_rate_b,60,100);
